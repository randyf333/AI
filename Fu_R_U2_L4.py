...
def ordered_domain(var_index, variables, q_table):
   ...
   if var_index in variables:
      ...
      sortedq = sorted(q_table.items(),key = lambda item: item[1])
      for x in sortedq:
...
def recursive_backtracking(assignment, variables, neighbors, q_table):
   ...
   #variables
   # if there are possibilites for the selected optimal dot idex 
   #    remove that dot index formo dictionary 
   #    for symbol in symbols:
   #       make copy 
   #       update dictionary 
   #       create subpzl 
   #       call recursive 
   #       if recursive returns a pzl then return that pzl 
   ...
   for x in values:
      ...
      newvariables = update_variables(x,var,variables,neighbors,newassignment)#,assign) # added assignment
      # global count
      # count += 1
