...
def utility(turn, tc, state):
   ...
   for group in tc:
      checklist = [state[x] for x in group]
      if checklist == [turn,turn,turn]:
         return 1
      elif turn not in checklist and '.' not in checklist:
         return -1 
   return 0 
...
def max_value(state, turn, tc):
   ...
   value = (-math.inf)
   nex = successors(state,get_turn(state))
   for a,s in nex:
      nextstate = min_value(s,turn,tc)
      if nextstate[0] > value:
         value = nextstate[0]
         state = s
      #value = max(value,nextstate[0])
