...
def searchNext(goal, heuristic, costDict,visited, steps, current, parent, depth):
    
    cost = heuristic(current,goal) + steps   
    pathsInfo =[cost,steps,current,parent]
    steps += 1
    inDic = False
    # if current == goal and depth == 0:
    #     return pathsInfo
    if current in visited:
        oldpathInfos = visited[current]
        if cost >= oldpathInfos[0]:
            inDic = True
        else:
            print("new cost")   
    else:
        visited[current]= pathsInfo

    if not inDic:
        if cost in costDict:
            pathsInfos = costDict[cost]
            pathsInfos.append(pathsInfo)
        else:
            costDict[cost] = [pathsInfo]
    ...
    for kid in children:
        if kid in visited:
            continue
        ret = searchNext(goal,heuristic,costDict,visited, steps, kid, pathsInfo, depth)
        if ret is not None:
            return ret
