...
from math import exp, pi, acos, sin, cos
from tkinter import *
import heapq
...
class HeapPriorityQueue():
   ...
   def __init__(self):
      #self.queue = ["dummy"]
      self.queue = [[math.inf,math.inf,"dummy"]]
...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   # Your code goes here
   nodeFile = open(nodes,"r")
   for line in nodeFile.readlines():
       vertex, latitude, longitude = line.strip().split()
       nodeLoc[vertex] = (latitude,longitude)
   nodeFile.close()
   cityFile = open(node_city,"r")
   for city in cityFile.readlines():
       vertex, name = city.strip().split(maxsplit=1)
       nodeToCity[vertex] = name
       cityToNode[name] = vertex
   cityFile.close()
   connectFile = open(edges)
   for connection in connectFile.readlines():
       city1, city2 = connection.strip().split()
       edgeCost[(city1,city2)] = calc_edge_cost(nodeLoc[city1][0],nodeLoc[city1][1],nodeLoc[city2][0],nodeLoc[city2][1])
       edgeCost[(city2,city1)] = calc_edge_cost(nodeLoc[city1][0],nodeLoc[city1][1],nodeLoc[city2][0],nodeLoc[city2][1])
       if city1 in neighbors:
          neighbors[city1].add(city2)
       else:
          neighbors[city1] = {city2}
   
       if city2 in neighbors:
          neighbors[city2].add(city1)
       else:
          neighbors[city2] = {city1}
...
def display_path(path, graph):
   ...
   for item in path:
       if item in graph[1]:
           pathList.append(graph[1][item])
   print("The whole path: "+str(path))
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   ...
   for p in range(len(path)-1):
       drawLine(canvas, *graph[5][path[p]], *graph[5][path[p+1]], 'red')
       ROOT.update()
...
def bi_bfs(start, goal, graph, col):
   ...
   counter = 0
   sfrontier, explored = [],{start: 's'}
   efrontier, eexplored = [],{goal: 's'}
   sfrontier.append(start)
   efrontier.append(goal)
   while sfrontier and efrontier:
       s = sfrontier.pop(0)
       if s in eexplored:
           path, cost = generate_path(s, explored, graph)
           epath, ecost = generate_path(s, eexplored, graph)
           totalpath = path[:-1]+epath[::-1]
           totalcost = cost+ecost
           draw_final_path(ROOT, canvas, totalpath, graph)
       ...
       e = efrontier.pop(0)
       if e in explored:
           path, cost = generate_path(e, explored, graph)
           #print(path)
           epath, ecost = generate_path(e, eexplored, graph)
           totalpath = path[:-1] + epath[::-1]
           totalcost = cost + ecost
           #print(epath)
           #totalpath = epath
           ...
           #return totalpath, totalcost
       for b in graph[3][e]:
           if b not in eexplored:
               eexplored[b] = e
               efrontier.append(b)
               drawLine(canvas, *graph[5][e], * graph[5][b], col)
       counter += 1
       if counter % 1000 == 0: ROOT.update()
   return None
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   counter = 0
   frontier, explored = HeapPriorityQueue(),{start: "s"}
   x = [heuristic(start,goal,graph),0,start]
   frontier.push(x)
   while frontier:
      current_data = frontier.pop()
      f_current, g_current, current = current_data
      if current == goal:
         path, cost = generate_path(current,explored,graph)
         draw_final_path(ROOT,canvas,path,graph)
         return path, cost
      for c in graph[3][current]:
         if c not in explored:
            explored[c] = current
            gcost = graph[4][(current,c)]+g_current
            fcost = heuristic(c,goal,graph)+gcost
            node = [fcost,gcost,c]
            frontier.push(node)
            drawLine(canvas,*graph[5][current],*graph[5][c],col)
      counter += 1
      if counter % 1000 == 0: ROOT.update()
   return None
...
def bi_a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   counter = 0
   frontier, explored = HeapPriorityQueue(),{start: "s"}
   bfrontier, bexplored = HeapPriorityQueue(),{goal:"s"}
   x = [heuristic(start,goal,graph),0,start]
   frontier.push(x)
   y = [heuristic(goal,start,graph),0,goal]
   bfrontier.push(y)
   while frontier and bfrontier:
      current_data = frontier.pop()
      f_current, g_current, current = current_data
      if current in bexplored:
         path, cost = generate_path(current, explored, graph)
         epath, ecost = generate_path(current, bexplored, graph)
         totalpath = path[:-1]+epath[::-1]
         totalcost = cost+ecost
         draw_final_path(ROOT, canvas, totalpath, graph)
         return totalpath, totalcost
      for c in graph[3][current]:
         if c not in explored:
            ...
            drawLine(canvas,*graph[5][current],*graph[5][c],col)
      counter += 1
      bcurrent_data = bfrontier.pop()
      bf_current, bg_current, bcurrent = bcurrent_data
      if bcurrent in explored:
         path, cost = generate_path(bcurrent,explored,graph)
         epath, ecost = generate_path(bcurrent, bexplored, graph)
         totalpath = path[:-1]+epath[::-1]
         totalcost = cost+ecost
         draw_final_path(ROOT, canvas, totalpath, graph)
         return totalpath, totalcost
      for d in graph[3][bcurrent]:
         if d not in bexplored:
            bexplored[d] = bcurrent
            gcost = graph[4][(bcurrent,d)]+bg_current
            fcost = heuristic(d,start,graph)+gcost
            bnode = [fcost,gcost,d]
            bfrontier.push(bnode)
            drawLine(canvas,*graph[5][bcurrent],*graph[5][d],col)

      counter += 1
      if counter % 1000 == 0: ROOT.update()
   return None

def tri_helper(start,goal,graph,col,heuristic,ROOT,canvas):
   counter = 0
   frontier, explored = HeapPriorityQueue(),{start: "s"}
   bfrontier, bexplored = HeapPriorityQueue(),{goal:"s"}
   x = [heuristic(start,goal,graph),0,start]
...
def tri_directional(city1, city2, city3, graph, col, heuristic=dist_heuristic):
   ...
   path1, cost1 = tri_helper(city1,city2,graph,col,heuristic,ROOT,canvas)
   path2, cost2 = tri_helper(city3,city2,graph,col,heuristic,ROOT,canvas)
   path3, cost3 = tri_helper(city1,city3,graph,col,heuristic,ROOT,canvas)
   finalcost = 0
   finalpath = []
   costlist = [(cost1,path1,),(cost2,path2,),(cost3,path3,)]
   costlist.sort()
   finalcost += costlist[0][0]
   finalpath += costlist[0][1][:-1] 
   finalcost += costlist[1][0]
   finalpath += costlist[1][1]
   draw_final_path(ROOT,canvas,finalpath,graph,col)
